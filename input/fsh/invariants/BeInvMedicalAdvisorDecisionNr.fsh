...
Severity: #error
Expression: "Claim.subType.coding.where(system = 'https://www.ehealth.fgov.be/standards/fhir/eattestation/CodeSystem/be-cs-eattestation-types').where(code = 'physiotherapy' or code = 'speechtherapy').exists() implies Claim.item.count() <= Claim.item.extension.where(url = 'https://www.ehealth.fgov.be/standards/fhir/eattestation/StructureDefinition/be-ext-eattestation-item-preauthref').count()
                and
